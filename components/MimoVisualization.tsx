...
const MIMO3DVisualization = dynamic(() => Promise.resolve(({ antennaConfig, isSimulating }) => {
  ...
  useEffect(() => {
    ...
    const createDetailedAntenna = (color) => {
      ...
      // Antenna element
      const elementGeometry = new THREE.CylinderGeometry(0.02, 0.02, 1.2, 8);
    ...
    txAntennas.forEach(tx => {
      rxAntennas.forEach(rx => {
        const particles = createSignalParticles(
          new THREE.Vector3(tx.position.x, tx.position.y + 1.5, tx.position.z),
          new THREE.Vector3(rx.position.x, rx.position.y + 1.5, rx.position.z),
          0x88ccff
        );
      ...
      if (isSimulating) {
        ...
        particleSystemsRef.current.forEach(particleSystem => {
          ...
          for (let i = 0; i < particleSystem.count; i++) {
            ...
            positions[i * 3] = particleSystem.startPos.x + (particleSystem.endPos.x - particleSystem.startPos.x) * t;
            positions[i * 3 + 1] = particleSystem.startPos.y + (particleSystem.endPos.y - particleSystem.startPos.y) * t +
              Math.sin(t * Math.PI) * 0.5; // Arc height
            positions[i * 3 + 2] = particleSystem.startPos.z + (particleSystem.endPos.z - particleSystem.startPos.z) * t;
          }
